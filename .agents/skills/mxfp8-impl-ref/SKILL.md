...
## LLM 在线量化实现模式（参考 vllm-ascend）

与 Diffusion 模式的最大区别是：LLM 场景推理频次极高，**必须在加载时预置转置，避免每次推理都 `.transpose(0, 1)`**。

```python
# process_weights_after_loading
qw, w_scale = torch_npu.npu_dynamic_mx_quant(weight_fp, dst_type=torch_npu.float8_e4m3fn)
# 预转置并 contiguous，推理时直接用
...
# 兼容 shim，避免低版本 torch_npu 报错
_FLOAT8_E8M0FNU_DTYPE = getattr(torch_npu, "float8_e8m0fnu", getattr(torch, "float8_e8m0fnu", None))

# apply（推理）
# ...获取 qx 和 input_scale 同从前...
output = torch_npu.npu_quant_matmul(
    qx,
    layer.weight,           # ！！！不用 transpose
    layer.weight_scale_inv, # ！！！不用 transpose
...
- 在线实现 (Diffusion)：`sglang/.../multimodal_gen/runtime/layers/quantization/mxfp8_npu.py`
- 在线/离线实现 (LLM, vllm-ascend 风格)：`sglang/.../srt/hardware_backend/npu/quantization/linear_method_npu.py`
- 参考 (Diffusion)：`MindIE-SD/mindiesd/quantization/layer.py`
- 参考 (LLM)：`vllm-ascend/vllm_ascend/quantization/methods/w8a8_mxfp8.py`
