...
- **Ascend 950 强制 `allow_internal_format=False` → 离线 W4A8 weight 必须保持 ND，不能 cast 到 FRACTAL_NZ**（`junlin_qwen3_dense_w4a8`，commit `4c1a0f5a0d`，2026-06-24，用户机器 040202 权重 e2e 触发）：实测该 950 上 `torch_npu.npu.config.allow_internal_format = True` 被设备打回 False（warning `Current device only support allow_internal_format=False`，且**无对应 getter**，读会 `AttributeError`），所以 FRACTAL_NZ 内部格式根本不可用。`NPUMXFP4W4A8OfflineLinearMethod` 原本照搬 vllm-ascend 做 `npu_format_cast(weight, 29, customize_dtype=fp8, input_dtype=fp4)` → `.transpose(-1,-2)`，结果 `npu_quant_matmul`（`x2_dtype=float4_e2m1fn_x2`）报 **`x2 should be in the original image format or nz format, but it is 2`**。on-device 诊断（`format_cast→transpose` vs 各变体）结论：该 FP4 matmul **只接受 ND**（"original image format"）——transpose 后的 strided-NZ 被拒报 `it is 2`、contiguous NZ（任何朝向）被拒报 `it is 50`，**唯有「不 cast、只 transpose」的 ND view 通过**。修复：删掉 `process_weights_after_loading` 里的 `npu_format_cast`，只保留 `.transpose(-1,-2)`；FP4 解包由 `apply` 的 `x2_dtype + group_sizes` 驱动，与格式无关，数值不变。⚠️ 这台设备上 vllm-ascend 同样 `allow_internal_format=False` 却能跑通其 W4A8（其 `w4a8_mxfp4.py` 照样 cast NZ）——说明该 cast 在此硬件实为 no-op/有害，不要再照搬 vllm 的 NZ 写法。其它依赖 NZ 的路径（如在线 W4A8 `npu_dual_level_quant_matmul`）在此设备是否受同样限制待验。
