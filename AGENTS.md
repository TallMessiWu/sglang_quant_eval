...
- **离线 W4A8 (`W4A8_MXFP`) 在 A5 上有两个不同报错，根因完全不同，别混为一谈**（`junlin_qwen3_dense_w4a8`，2026-06-24/25，A5 + `Qwen3-8B-mxw4a8-pack-full-0421`，graph 模式 e2e 输出已验证正常）：
  ...
  - **② decode 阶段 `atb::OperationSetup` 段错误（伴 `Cannot create tensor with internal format while allow_internal_format=False`）= eager-decode 的 `ascend` attention bug，与量化无关**：升级 torch_npu 后改在 decode 崩。三段同步插桩定位：qkv 的 `npu_quant_matmul` 同步 OK、o_proj 的 matmul **还没跑**就在「进 o_proj 前的入口同步」崩 → fault 来自 qkv→o_proj 之间的 **decode attention**，不是 FP4 matmul。**真因是 `--disable-cuda-graph` 强制 eager decode**：NPU `set_default_server_args` 本就预设 decode 走 npu-graph（vllm 也只用 aclgraph 跑 decode），eager decode 这条路 `ascend` attention 没走通。**解法：别加 `--disable-cuda-graph`**，graph 模式下离线 W4A8 输出正常。此 attention bug **非 W4A8 特有**（与 linear 量化无关、在线/离线同理），属独立 NPU attention 问题，不在本 PR 交付范围。
