...
- **离线 W4A8 (`W4A8_MXFP`) 在 A5 上报 `x2 ... it is 2` / `atb::OperationSetup` 段错误 = 旧 torch_npu 的 FP4 `npu_quant_matmul` bug，升级 torch_npu 解决，代码保持 vllm 的 NZ 写法**（`junlin_qwen3_dense_w4a8`，2026-06-24，A5 + `Qwen3-8B-mxw4a8-pack-full-0421`）：
  - 现象：`NPUMXFP4W4A8OfflineLinearMethod` 照搬 vllm `npu_format_cast(weight,29,customize_dtype=fp8,input_dtype=fp4)` → `.transpose(-1,-2)`，`npu_quant_matmul(x2_dtype=float4_e2m1fn_x2, group_sizes=[0,0,32])` 报 `x2 should be in the original image format or nz format, but it is 2`；改成 ND（去 cast）则 kernel 在 `atb::OperationSetup` 段错误 + `Cannot create tensor with internal format while allow_internal_format=False`。
  - 该 A5 **强制 `allow_internal_format=False`**（设 True 被打回，且无 getter、读会 `AttributeError`），但 `npu_format_cast(...,29)` 仍能造出 `FRACTAL_NZ_C0_16`，不是阻塞点。
  - **真因是 torch_npu 版本**：独立诊断脚本（cast29+transpose → matmul）在 **`torch_npu 2.10.0.dev20260320`** 上复现 `it is 2`，升级到 **`2.10.0.post1.dev20260624`** 后 NZ / ND 各变体**全部 OK**。`npu_dynamic_mx_quant` 原生即返回 3D block scale `[tokens, in//64, 2]`，无需 vllm `maybe_normalize_mxfp_scale_layout` 那种 2D→3D normalize（那个只在 MoE 用）。
  - **结论 / 正确做法**：代码保持 **vllm 对齐的 NZ 写法**（cast29 + transpose），不要切 ND；遇到 `it is 2` / atb 段错误**先升级 torch_npu**。我中途基于旧 torch_npu 误判加的 ND commit `4c1a0f5a0d` 已回退。
