...
- **在线 W4A8 权重 FP4 `npu_dynamic_mx_quant` 在 torch_npu `2.10.0.post2.dev20260704` 被整体打回，用纯 torch RTN 正向绕过**（`junlin_qwen3_dense_w4a8`，2026-07-06，commit `db4dee06ea`）：跑 `mxfp_w4a8` 在线量化时 `process_weights_after_loading` 里 `torch.ops.npu.npu_dynamic_mx_quant(weight, dst_type=float4_e2m1fn_x2, round_mode="round")` 崩「output y must be same shape as input x」（error 561002，tiling `dynamic_mx_quant_tiling_arch35.cpp:343`）。**这是纯 upstream torch_npu FP4 kernel regression，不是我们的 call/shape/布局问题**：最小 probe 在 A5（CANN 9.1.0）证实——`dst=fp8` 恒 OK，**所有 fp4 shape（weight `[4096,4096]`、activation `[128,4096]`、任意 K）全崩**，连裸 vllm-style 调用也崩；旧版 `2.10.0.post1.dev20260624`（07-02 用过）能跑，新版打回。该盒子已升到最新、**无法降级**，Python 侧不可修（不是漏传 kwarg、不是 shape 对齐、`mxfp_compat.py` 也只有 dtype 存在性检查、无 call-site 版本 shim）。
  - **区分「解决 vs 规避」的教训**：我一开始没证明 op 真坏就直接跳到纯 torch workaround，被用户纠正「你应该解决报错，不是规避它」。正确姿势是先写 probe 坐实是**不可修的上游 regression**（fp8 OK / 全 fp4 崩 / 不可降级），此时纯 torch 才从「规避」升格为「正向解」——何况离线路径本就用纯 torch `pack_fp4_to_uint8`。
  - **修法**：`linear_method_npu.py` 新增纯 torch helper `_mxfp4_quantize_weight(weight_fp)`，复刻 msmodelslim MXFP4 RTN：block=32、每块 shared exp `floor(log2(amax))-2`（emax=2）、E2M1 grid `[0,.5,1,1.5,2,3,4,6]` 用 midpoint 边界 `(.25,.75,1.25,1.75,2.5,3.5,5.0)` 计数出 magnitude index、符号位 bit3、`codes[...,0] | codes[...,1]<<4` pack 成 uint8 `[out,in//2]`、scale `(exp+127)` UE8M0 uint8 `[out,in//32]`。产出**与离线 `W4A8_MXFP` checkpoint 逐字节同格式**，因此直接复用离线已验证的 `npu_format_cast(...,NZ,customize_dtype=fp8,input_dtype=fp4)` → transpose → `npu_quant_matmul(x2_dtype=fp4, group_sizes=[0,0,32])` 路径。**激活量化不动**（`dst=fp8` 不受 regression 影响）。
  - **验证边界**：本地无 torch/NPU，仅 py_compile + 纯 Python 量化数学交叉校验（500k 采样 midpoint-index vs msmodelslim argmin **0 mismatch**、pack 往返 OK、零块 scale=127）。**A5 端到端仍待用户重跑 `qwen3_dense_online_w4a8.sh` 确认输出连贯（非乱码）并与离线对齐。**⚠️ 与「NPU 崩溃先分清类别」memory 互补：那条说「prefill format 报错=升级 torch_npu 修好」，本条是**升级把另一个 FP4 op 反向搞坏且不可降级**，方向相反，别照搬「升级就好」的结论。
