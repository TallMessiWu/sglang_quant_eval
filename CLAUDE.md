...
加载 msmodelslim 导出的 MXFP4 权重（FP4 packed + uint8 scale + bfloat16 dual scale）。使用 `npu_dual_level_quant_matmul` 双级量化矩阵乘。

| 文件 | 操作 | 说明 |
|------|------|------|
| `sglang/.../multimodal_gen/runtime/layers/quantization/modelslim_mxfp4_scheme.py` | **新增** | `ModelSlimMXFP4Scheme` — 加载 MXFP4 预量化权重，支持双级 scale |
