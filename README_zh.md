...
- `sglang/` - 本地 `git worktree` 容器，将 SGLang fork 的 2 个活跃分支同时 checkout。`qwen3_dense_w4a4/` 是主 clone **且是子模块**（GitHub 上可点击跳转至 fork 的 `junlin_qwen3_dense_w4a4`）；`qwen3_moe_w8a8/` 是派生 worktree（纯本地、gitignore）。已合并功能（Diffusion MXFP8/MXFP4、Dense W8A8/W4A8）都在 upstream/main。worktree 与分支的对应关系、已合并 PR 清单见 `AGENTS.md`。
