...
> 最近核对：2026-09-14（Asia/Shanghai）；#32745 与 #638 两行于 2026-09-17 合并上游后重新核对；sgl-kernel-npu #808 于 2026-09-17 新建后核对；SGLang #40419 与 kernel 分支 `gdn-state-dtype-check` 于 2026-09-20 新建后核对，#32745 的 head 同日回退到 `4f35f3ef7e`；同日稍后按用户要求把 bf16 断言并入 #808、把 #40419 叠到 `junlin_qwen3.5_dense_w8a8_pr36426`，并重做两个 kernel 构建分支的 merge；同日 #32745 与该测试分支合入 `upstream/main` `c1a1eb5f66`（CONFLICTING → MERGEABLE）。GitHub 状态来自实时 PR 查询，本地状态来自 `git worktree list --porcelain`、`git branch --show-current` 和 `git rev-parse HEAD`。这些信息会漂移，操作前必须重新查询。2026-09-24 按用户要求复核 sgl-kernel-npu 侧 #808/#823 两对共四个分支：GitHub 两个 PR 均为 OPEN + Draft + MERGEABLE（本次已算出 mergeable，不再是 UNKNOWN），本地 head 与 GitHub head 一致，四个分支与 `origin` 零分叉、worktree 干净；**SGLang 侧 PR 状态本轮未复核**。同时更正了「两个实验分支只差 195 行」这条已过期的结论（见下文）。2026-09-24 稍后：按用户决定 **#808 保持整份不拆分**，已取消 Draft 转为 ready for review（`isDraft=false`，`BLOCKED` 仅因缺 approve；该分支无 CI checks，也未指定 reviewer）；同日补上 maintainer 要求的单算子性能用例 `tests/python/sgl_kernel_npu/test_recurrent_gated_delta_rule_vs_triton.py`（先 `ffa0542` 提出，同日把三轮修正 squash 成单提交，随后两次 amend：去掉测试文件里固化的实测数据表、精简 docstring 与注释 410→370 行 / docstring 62→33 行 / `#` 注释 25→15 行），现 head `22ec11275c`，分支共 4 提交。A/B 实测表写在 PR 描述里、不固化在测试文件中，因为数字每跑一轮都在动）并把判据实测结果写入下文。**另注意：该分支 2026-09-24 经 squash 与两次 amend 共三次 force-push 重写，之前 pull 过的 checkout 必须 `git reset --hard origin/ascend950-recurrent-gated-delta-rule`，不要 `git pull`。**
...
| sgl-kernel-npu [#808](https://github.com/sgl-project/sgl-kernel-npu/pull/808) — recurrent_gated_delta_rule on Ascend 950 | Open，MERGEABLE / BLOCKED（`REVIEW_REQUIRED`） | `ascend950-recurrent-gated-delta-rule` | `22ec11275c`（4 提交：950 适配 + bf16 state 断言 + 合上游 + triton A/B 性能用例；2026-09-24 三次 force-push 后） | `004bc36`（2026-09-20） | `sgl-kernel-npu-worktrees/ascend950-recurrent-gated-delta-rule/` |
| sgl-kernel-npu ~~[#823](https://github.com/sgl-project/sgl-kernel-npu/pull/823)~~ — 同一算子，只放开编译（#808 的严格子集，对照用） | **Closed**（2026-09-24，由 #808 承接） | `ascend950-rgdr-ungate-probe` | `26c905a`（只改 3 个注册文件，+19/-19；分支与 worktree 保留，远端分支未删） | `004bc36`（2026-09-23） | `sgl-kernel-npu-worktrees/ascend950-rgdr-ungate-probe/` |
...
**2026-09-24 判据落地（两个 wheel 各跑 `--matrix` + 跨 wheel `--compare`）**：bf16（连续/strided 两种布局 × 4 组形状，到 MTP=8）两边全部 `超容差元素=0`，对 golden 的最大误差**逐格相同**（out `9.500~9.763e-04`，state `1.953e-03~3.850e-03`），其中 state `3.835e-03` 距容差绝对下限 `2⁻⁸=3.906e-03` 只剩 **1.8%** 余量。两边彼此的差是 bf16 单 ULP（`1.953e-03`，连续与 strided 都复现同样 6 项），属归约顺序不同而非算法差异；`--matrix` **分辨不出**这 1 ULP（误差峰值由两边共有的 bf16 量化主导），只有 `--compare` 能。**真正的分水岭是断言**：#808 对 fp32 state 直接 `RuntimeError: recurrent_state must be bfloat16`；#823 无断言，静默算出 1e35/NaN（MTP=8 时 `14886654/18874368` 个元素坏，即已踩过的那个「输出通顺但逐渐失忆」故障模式回来）。单算子耗时 #808 快 1.4~4.6×（连续 `1.43/2.17/3.33/4.37`，strided `1.59/2.09/3.21/4.60`；同 wheel 重跑的噪声基线 ≤14%，故全部在带外），属微基准，e2e 收益未测。**决定：不拆分，走整份 #808。**#823 已于 2026-09-24 关闭（关闭评论里带上了上面的实测结论），分支与 worktree 保留、远端分支未删；未收尾项：`ascend950-mtp-experiment-ungate` 那份 wheel 的构建目标仍未确认（`hasattr(torch.ops.npu,'kv_compress_epilog')` 或构建日志的 `SGL_KERNEL_ARCH=`）——若为原生 950，则说明 arch22 向量块 API 在 arch35 上仍能编能跑、只是慢，这是一个可写进 PR 的附带结论。
