...
| SGLang [#40419](https://github.com/sgl-project/sglang/pull/40419) — 修 Qwen3.5 GDN 在 Ascend 上的 prefill 与投机解码 | Open，MERGEABLE | `junlin_npu_mamba_ssm_dtype` | `c0932108e9`（2 提交：bf16 SSM state + gating 补维；两笔都 cherry-pick 到 `junlin_qwen3.5_dense_w8a8_pr36426`） | `2d216a11f8`（2026-09-20） | `sglang/npu_mamba_ssm_dtype/` |
...
| 仓 | 目录 | 分支 | HEAD | 叠了什么 |
| --- | --- | --- | --- | --- |
| SGLang | `sglang/qwen3.5_dense_w8a8_pr36426/` | `junlin_qwen3.5_dense_w8a8_pr36426` | `62644c2253` | #32745 + 上游 #36426 + #40419（cherry-pick），base `c1a1eb5f66` |
...
**#747 的第三处断裂：prefill 的 state 布局（2026-09-20，Qwen3.5 MTP 在 950 上最终跑通）**：#747 把 pool、decode triton、verify 算子统一成 `(nv, dv, dk)`，**但没有动 prefill 走的 `sgl_kernel_npu.fla.chunk`**，它仍返回 `(nv, dk, dv)`。SGLang 的 `ssm_states[cache_indices] = last_recurrent_state` 因此把转置后的内容写进 pool —— `dk == dv == 128` 时形状相同，不报错。用 `dk=64, dv=32` 实测（`llm/gdn_prefill_layout_check.py`，模型自身的形状永远分辨不出来）：triton 返回 `(1,4,64,32)`、#747 新增的 AscendC 算子返回 `(1,4,32,64)`，两者 attention 输出一致（128 token 时差 3.6e-02，bf16 累积量级）。修法是让 prefill 改调 `torch.ops.npu.chunk_gated_delta_rule`（`llm/patch_gdn_prefill_ascendc.py`，已提交进测试分支 `62644c2253`）。**这一处加上前两处（bf16 state、gating 降维）之后，950 上的 Qwen3.5 MTP 才真正跑通。****两个已知妥协**，还不够上游质量：入参是 `mixed_qkv` 的跨步视图，补了 `.contiguous()` 拷贝（不然 `l2norm_fwd` 的 `view()` 直接报错）；`h` 返回 `None`，于是 `_track_mamba_state_extend` 被跳过 —— **mamba 的 page 跟踪因此失效，前缀缓存命中时复用的 state 可能是错的**，算子的 `chunk_state` 参数就是该接到这里的。离线比对里还有一处没解释清楚：两个实现的 state 转置后仍对不上（相对差 1.0），但 attention 输出一致，暂未追。
