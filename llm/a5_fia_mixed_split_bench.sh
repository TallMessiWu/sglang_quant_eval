...
#
# 默认负载是按"让 prefill 和 decode 持续重叠"设计的 —— 拆分只在这种时候
# 才起作用：
#   * INPUT_LEN 远大于服务端的 --chunked-prefill-size，每个请求要好几个
#     分块才做完 prefill；
#   * OUTPUT_LEN 足够长，先到的请求还在 decode 时后到的请求正在 prefill；
#   * MAX_CONCURRENCY 足够大，mixed batch 里的 decode 请求不止一个。
# decode 请求只有 1 个时，拆分多出来的那次 kernel launch 很可能吃掉全部
# 收益，测出来会是负的。
...
环境变量覆盖:
  ...
  NUM_PROMPTS      请求总数        (默认 64)
  INPUT_LEN        输入长度        (默认 4096，必须大于服务端分块大小)
  OUTPUT_LEN       输出长度        (默认 512)
  MAX_CONCURRENCY  并发上限        (默认 16)
...
PY
    ...
    *)
        echo "❌ 无效动作: $action (应为 run 或 compare)" >&2
        usage
        exit 2
        ;;
esac
