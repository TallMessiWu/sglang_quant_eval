"""
Wan2.2 TI2V (Text-Image-to-Video) Online MXFP4 量化推理脚本
使用 SGLang DiffGenerator API + 路径 A (在线 MXFP4 双级量化)

无需预量化权重 — 从原始 FP16/BF16 模型在加载时自动量化为 MXFP4，
利用 torch_npu.npu_dynamic_dual_level_mx_quant + npu_dual_level_quant_matmul 进行推理。
...
def main():
    parser = argparse.ArgumentParser(description="Wan2.2 TI2V Online MXFP4 双级量化视频生成")
    ...
    parser.add_argument("--seed", type=int, default=42,
                        help="随机种子")
    parser.add_argument("--output-dir", type=str, default="./outputs_mxfp4_online",
                        help="输出目录")
    ...
    print(f"量化方式: Online MXFP4 双级量化 (W4A4)")
    ...
    # 初始化生成器
    # quantization="mxfp4" 使 transformer 的所有 Linear 层自动在线双级量化为 MXFP4
    print("正在加载模型（Online MXFP4 双级量化）...")
