"""
Wan2.2 TI2V (Text-Image-to-Video) W8A8 INT8 量化推理脚本
使用 SGLang DiffGenerator API + msmodelslim 预量化权重
量化模型: Wan2.2-TI2V-5B-w8a8c8-self-attn-bf16-rot
"""
...
def main():
    ...
    parser.add_argument("--model-path", type=str, default="/home/weights/Wan2.2-TI2V-5B-Diffusers",
                        help="原始模型路径（用于 VAE、文本编码器等非量化组件）")
    parser.add_argument("--quant-model-path", type=str,
                        default="/home/weights/Wan2.2-TI2V-5B-w8a8c8-self-attn-bf16-rot",
    ...
    # 确认量化模型路径存在
    if not os.path.isdir(args.quant_model_path):
        print(f"错误: 量化模型路径不存在: {args.quant_model_path}")
        return

    quant_desc = os.path.join(args.quant_model_path, "quant_model_description.json")
    if not os.path.exists(quant_desc):
        print(f"警告: 未找到 quant_model_description.json，量化可能无法自动生效: {quant_desc}")

    print(f"原始模型: {args.model_path}")
    print(f"量化 transformer: {args.quant_model_path}")
    ...
    # 初始化生成器
    # component_paths 覆盖 transformer 组件路径，指向量化权重目录
    # diffusion 子系统会自动检测目录内的 quant_model_description.json 并启用 modelslim 量化
    print("正在加载模型（W8A8 INT8 量化）...")
